...
// compares 2 stacks. True if each corresponding value is the same
// and in the same position for each stack. False otherwise
template <class Type>
bool stackType<Type>::operator==(const stackType &obj)
{
    if(this->stackTop != obj.stackTop)
        return false; // compare the stackTops first

    // compare each element starting from the stackTop
